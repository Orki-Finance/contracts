...
abstract contract BaseZapper is AddRemoveManagers, LeftoversSweep, IFlashLoanReceiver, IZapper, AccessManaged {
    ...
    constructor(IAddressesRegistry _addressesRegistry, IFlashLoanProvider _flashLoanProvider, IExchange _exchange)
        ...
        AccessManaged(_addressesRegistry.authority())
    {
    ...
    function setProviders(IFlashLoanProvider _flashLoanProvider, IExchange _exchange) public virtual restricted {
        require(address(flashLoanProvider) == address(0) || address(_exchange) == address(0), "Providers already set");
        require(
            address(_flashLoanProvider) != address(0) && address(_exchange) != address(0), "Providers cannot be zero"
        );
    ...
    function _requireProviders() internal view {
        if (address(flashLoanProvider) == address(0) || address(exchange) == address(0)) {
            revert FeatureNotAvailable();
        }
    }
}
