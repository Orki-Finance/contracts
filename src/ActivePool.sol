...
contract ActivePool is IActivePool {
    ...
    address public immutable collateralRegistryAddress;
    ...
    uint256 public stabilityPoolYieldSplit; // SP yield split
    ...
    */
    ...
    error SPYieldSplitOutOfBounds();
    error ZeroAddressInterestRouter();

    event SPYieldSplitChanged(uint256 _newSplit);
    event InterestRouterChanged(address _newInterestRouter);

    constructor(IAddressesRegistry _addressesRegistry, uint256 _stabilityPoolYieldSplit) {
        _requireValidSPYieldSplit(_stabilityPoolYieldSplit);
        ...
        collateralRegistryAddress = address(_addressesRegistry.collateralRegistry());

        stabilityPoolYieldSplit = _stabilityPoolYieldSplit;
    ...
    function setSPYieldSplit(uint256 _newSplit) external {
        _requireCallerIsCollateralRegistry();
        _requireValidSPYieldSplit(_newSplit);

        stabilityPoolYieldSplit = _newSplit;

        emit SPYieldSplitChanged(_newSplit);
    }

    function setInterestRouter(address _newInterestRouter) external {
        _requireCallerIsCollateralRegistry();
        if(_newInterestRouter == address(0)){
            revert ZeroAddressInterestRouter();
        }

        interestRouter = IInterestRouter(_newInterestRouter);

        emit InterestRouterChanged(_newInterestRouter);
    }
    ...
    */
    ...
    function _mintAggInterest(uint256 _upfrontFee) internal returns (uint256 mintedAmount) {
        ...
        if (mintedAmount > 0) {
            uint256 spYield = stabilityPoolYieldSplit * mintedAmount / DECIMAL_PRECISION;
    ...
    function _requireCallerIsBOorSPorCR() internal view {
        require(
    ...
    function _requireCallerIsCollateralRegistry() internal view {
        require(msg.sender == collateralRegistryAddress, "ActivePool: Caller is not CollateralRegistry");
    }

    function _requireValidSPYieldSplit(uint256 _stabilityPoolYieldSplit) internal pure {
        if (_stabilityPoolYieldSplit == 0 || _stabilityPoolYieldSplit > _100pct) revert SPYieldSplitOutOfBounds();
    }
}
