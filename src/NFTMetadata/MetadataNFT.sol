...
contract MetadataNFT is IMetadataNFT {
    ...
    function uri(TroveData memory _troveData) public view returns (string memory) {
        ...
        return json.formattedMetadata(
            string.concat("Orki - ", IERC20Metadata(_troveData._collToken).name()),
            string.concat(
                "Orki is a collateralized debt platform. Users can lock up ",
                IERC20Metadata(_troveData._collToken).symbol(),
                " to issue stablecoin tokens (USDK) to their own Swellchain address. The individual collateralized debt positions are called Troves, and are represented as NFTs."
            ),
