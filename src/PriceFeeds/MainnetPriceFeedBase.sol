...
abstract contract MainnetPriceFeedBase is IMainnetPriceFeed, AccessManaged {
    ...
    function setAddresses(address _borrowOperationsAddress) external restricted {
        if (address(borrowerOperations) != address(0)) revert AddressesAlreadySet();
    ...
    function _shutDownAndSwitchToLastGoodPrice(address _failedOracleAddr) internal returns (uint256) {
        ...
        borrowerOperations.shutdownFromOracleFailureOrGovernance();
