...
contract TroveManager is LiquityBase, ITroveManager, ITroveEvents {
    ...
    uint256 public minAnnualInterestRate;
    ...
    uint256 public liquidationPenaltyRedistribution;
    ...
    **/
    ...
    constructor(
        ...
        _requireValidConfig(
            _ccr, _mcr, _scr, _bcr, _liquidationPenaltySP, _liquidationPenaltyRedistribution, _minAnnualInterestRate
        );
        if (_minDebt < MIN_ALLOWED_DEBT) revert InvalidParam();
        ...
        BCR = _bcr;
        MIN_DEBT = _minDebt;
        minAnnualInterestRate = _minAnnualInterestRate;
        liquidationPenaltySP = _liquidationPenaltySP;
        liquidationPenaltyRedistribution = _liquidationPenaltyRedistribution;
        ...
        emit BranchConfigurationUpdated(
            _scr, _mcr, _ccr, _bcr, _liquidationPenaltySP, _liquidationPenaltyRedistribution, _minAnnualInterestRate
        );
    }
    ...
    function getBorrowerOperations() external view virtual returns (IBorrowerOperations) {
        return borrowerOperations;
    }

    // --- Setters ---

    // Warning: future changes to ratio parameters should have the following in mind:
    //      - Check https://github.com/liquity/bold/issues/496
    function setNewBranchConfiguration(
        ...
        _requireValidConfig(
            ...
            _scr,
            _bcr,
            _newLiquidationPenaltySP,
            _newLiquidationPenaltyRedistribution,
    ...
    */
        ...
        )
    {
        ...
        if (_boldInSPForOffsets > 0) {
            ...
            (collToSendToSP, collSurplus) =
                _getCollPenaltyAndSurplus(collToOffset, debtToOffset, liquidationPenaltySP, _price);
        }
    ...
    function batchLiquidateTroves(uint256[] memory _troveArray) public override {
        ...
        //activeColl
        activePoolCached.getCollBalance() - totals.collGasCompensation
        //liquidatedColl
        + defaultPool.getCollBalance();
    ...
    */
    function redeemCollateral(
        ...
        if (totalsTroveChange.collDecrease > 0) {
    ...
    function urgentRedemption(uint256 _boldAmount, uint256[] calldata _troveIds, uint256 _minCollateral) external {
        ...
        if (boldToken.balanceOf(msg.sender) < _boldAmount) {
    ...
    */
    ...
    function _requireValidConfig(
        ...
        if (!(_scr > _100pct && _scr <= _mcr && _mcr < _ccr && _ccr < _200pct)) revert InvalidParam();
        if (
            _newLiquidationPenaltySP < MIN_LIQUIDATION_PENALTY_SP || _newLiquidationPenaltyRedistribution == 0
                || _newLiquidationPenaltySP > _100pct
                || _newLiquidationPenaltyRedistribution > MAX_LIQUIDATION_PENALTY_REDISTRIBUTION
        ) {
            revert InvalidParam();
        }
        if (
            _minAnnualInterestRate < MIN_POSSIBLE_ANNUAL_INTEREST_RATE
                || (_minAnnualInterestRate > minAnnualInterestRate && minAnnualInterestRate != 0)
        ) {
            revert InvalidParam();
        }
        if (_bcr < 5e16 || _bcr >= 50e16) {
            revert InvalidParam();
        }
    }
        ...
        if (debtIncrease == 0 && debtDecrease == 0) {
            ...
            if (debtIncrease > 0) {
                ...
                if (_batchDebt == 0) {
                    ...
                    if (currentBatchDebtShares * MAX_BATCH_SHARES_RATIO < _batchDebt && _checkBatchSharesRatio) {
