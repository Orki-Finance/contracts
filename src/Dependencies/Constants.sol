...
// Minimum amount of net Bold debt the protocol allows a trove must have
// The actual amount is defined per collateral,
// but can never be lower than this constant
uint256 constant MIN_ALLOWED_DEBT = 200e18;

uint256 constant MIN_POSSIBLE_ANNUAL_INTEREST_RATE = _1pct / 2; // 0.5%

// we can't use math operators on the constant declaration, since that puts us above contract size limits
// so this constant is hardcoded instead of depending on _1pct. To safeguard, a forge test was added to ensure the relationship remains the same and future refactors don't break anything
uint256 constant MAX_ANNUAL_INTEREST_RATE = 350e16;

// Batch management params
uint128 constant MAX_ANNUAL_BATCH_MANAGEMENT_FEE = uint128(_100pct / 10); // 10%
uint128 constant MIN_INTEREST_RATE_CHANGE_PERIOD = 1200 seconds; // prevents adjusting interest rates too frequently
...
uint256 constant MAX_NUMBER_COLLATERALS = 15;

uint256 constant MINIMUM_MAX_CAP_PER_BRANCH = MIN_ALLOWED_DEBT * 10;
