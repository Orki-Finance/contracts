...
import "./Quill/BorrowerOperationsSupportingLib.sol";
...
contract BorrowerOperations is LiquityBase, AddRemoveManagers, IBorrowerOperations {
    ...
    ISequencerSentinel internal immutable sequencerSentinel;
    ...
    error CallerNotPriceFeedOrCollateralRegistry();
    ...
    event SequencerSentinelChanged(address _sequencerSentinelAddress);
        ...
        LiquityBase(_addressesRegistry)
    {
        ...
        emit SequencerSentinelChanged(address(sequencerSentinel));
    ...
    function _openTrove(
        ...
        BorrowerOperationsSupportingLib.pullCollAndSendToActivePool(vars.activePool, _collAmount, collToken);
    ...
    function adjustTrove(
        ...
        // If the trove has a chance of decreasing health, a stricter sequencer check is enforced
        // this may yield some false positives (e.g.: if both debt and coll increase, but with a healthier final amount), but this check is much cheaper than computing the actual ICR before & after the call
        if (troveChange.debtIncrease > 0 || troveChange.collDecrease > 0) {
            _requireSequencerUpAndOverGracePeriod();
        } else {
            _requireSequencerUp();
        }
    ...
    function adjustZombieTrove(
        ...
        ITroveManager troveManagerCached = troveManager;
        // Quill: inlined `_requireTroveIsZombie`
    ...
    function _adjustTrove(
        ...
        uint256 minDebt = troveManager.MIN_DEBT();
        ...
        // Quill: inlined `_requireValidCollWithdrawal` to reduce contract size
        // the original condition `troveChange.collDecrease > 0` is already implicit in the check below (both values are unsigned)
        ...
        if (_troveChange.debtIncrease > 0) {
            _troveChange.upfrontFee = _calcUpfrontFee(
                _troveChange.debtIncrease, vars.activePool.getNewApproxAvgInterestRateFromTroveChange(_troveChange)
            );
        ...
        BorrowerOperationsSupportingLib.requireValidAdjustmentInCurrentMode(
            _troveChange, 
            vars.price, 
            vars.newICR,
            vars.isBelowCriticalThreshold, 
            isTroveInBatch);
    ...
    function applyPendingDebt(uint256 _troveId, uint256 _lowerHint, uint256 _upperHint) public {
        ...
        // Quill: inlined `_requireNonZeroDebt`
        ...
        if (_checkTroveIsZombie(troveManagerCached, _troveId) && trove.entireDebt >= troveManagerCached.MIN_DEBT()) {
            troveManagerCached.setTroveStatusToActive(_troveId);
            _reInsertIntoSortedTroves(
    ...
    function setBatchManagerAnnualInterestRate(
        ...
        if (
            ...
            batchChange.upfrontFee = _calcUpfrontFee(
                newDebt,
                // avgInterestRate
            ...
            _requireNewTCRisAboveCCR(_getNewTCRFromTroveChange(batchChange, price));
            // _requireOraclesLive()
        }
    ...
    function _applyUpfrontFee(
        ...
        _troveChange.upfrontFee =
            _calcUpfrontFee(_troveEntireDebt, activePool.getNewApproxAvgInterestRateFromTroveChange(_troveChange));
        ...
        _requireNewTCRisAboveCCR(
            // new TCR
            _getNewTCRFromTroveChange(_troveChange, price)
        );
    ...
    function shutdown() external {
        ...
        uint256 TCR = LiquityMath._computeCR(getEntireBranchColl(), getEntireBranchDebt(), price);
    ...
    // Quill: had to change this to allow governance shutdown as well. contract size space was on its limit
    // so a dedicated function was not feasible
    function shutdownFromOracleFailureOrGovernance() external {
        // inlined _requireCallerIsPriceFeed()
        if (msg.sender != address(priceFeed) && msg.sender != troveManager.getCollateralRegistry()) {
            revert CallerNotPriceFeedOrCollateralRegistry();
        }
    ...
    function _moveTokensFromAdjustment(
        ...
        if (_troveChange.collIncrease > 0) {
            // Pull coll tokens from sender and move them to the Active Pool
            BorrowerOperationsSupportingLib.pullCollAndSendToActivePool(
                _activePool, _troveChange.collIncrease, collToken
            );
    ...
    function _requireValidDelegateAdustment(
        ...
        if (individualDelegate.account == msg.sender) {
            ...
            // );
            if (block.timestamp < _lastInterestRateAdjTime + individualDelegate.minInterestRateChangePeriod) {
    ...
    function _requireAtLeastMinDebt(uint256 _debt) internal view {
        if (_debt < troveManager.MIN_DEBT()) {
            revert DebtBelowMin();
        }
    }
    ...
    function _requireValidAnnualInterestRate(uint256 _annualInterestRate) internal view {
        if (_annualInterestRate < troveManager.minAnnualInterestRate()) {
    ...
    // Cuurently checks if sequencer is up
    // If not, does not allow the current action (reverts)
