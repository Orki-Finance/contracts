...
contract RebasingBatchShares is DevTestSetup {
    ...
    function testBatchRebaseToSystemInsolvency() public {
        ...
        uint256 ATroveId = openTroveAndJoinBatchManager(A, 100 ether, mindebt, B, MAX_ANNUAL_INTEREST_RATE);
        ...
        uint256 BTroveId = openTroveAndJoinBatchManager(B, 100 ether, mindebt, B, MAX_ANNUAL_INTEREST_RATE);
        ...
        uint256 anotherATroveId = openTroveAndJoinBatchManagerWithIndex(
            A, x + 1, 100 ether, troveManager.MIN_DEBT(), B, MAX_ANNUAL_INTEREST_RATE
        );
    ...
    function _openCloseRemainderLoop(uint256 amt, uint256 BTroveId, uint256 iteration) internal {
        ...
        uint256 ATroveId = openTroveAndJoinBatchManagerWithIndex(
            A, iteration + 1, 100 ether, troveManager.MIN_DEBT(), B, MAX_ANNUAL_INTEREST_RATE
        );
    ...
    function testDeflateDebtLeavingSharesConstant() public {
        ...
        // B opens trove
        uint256 BTroveId =
            openTroveAndJoinBatchManager(B, 100 ether, troveManager.MIN_DEBT() - 2.3 ether, B, MAX_ANNUAL_INTEREST_RATE);
    ...
    function test_WhenBatchSharesRatioIsTooHigh_CanKickTroveFromBatch() external {
        registerBatchManager({
            _account: B,
            _minInterestRate: uint128(troveManager.minAnnualInterestRate()),
        ...
        for (uint256 i = 1;; ++i) {
            ...
            repayBold(A, targetTrove, troveManager.getTroveEntireDebt(targetTrove) - troveManager.MIN_DEBT());
            repayBold(A, placeholderTrove, troveManager.getTroveEntireDebt(placeholderTrove) - troveManager.MIN_DEBT());
        }

        // Make a zombie out of the target Trove
        skip(MIN_INTEREST_RATE_CHANGE_PERIOD);
        setBatchInterestRate(B, troveManager.minAnnualInterestRate());
        ...
        (uint256 liquidatedTrove,) = openTroveWithExactICRAndDebt({
            ...
            _ICR: MCR,
            _debt: 100_000 ether,
            _interestRate: troveManager.minAnnualInterestRate()
        });
        ...
        // The target Trove has more than troveManager.MIN_DEBT()
        assertGeDecimal(troveManager.getTroveEntireDebt(targetTrove), troveManager.MIN_DEBT(), 18, "debt < troveManager.MIN_DEBT()");
