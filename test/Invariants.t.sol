    ...
    function setUp() public override {
        ...
        if (n > 0) {
            p[0] = TestDeployer.TroveManagerParams(
                1.5 ether, 1.1 ether, 1.1 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
            );
        }
        if (n > 1) {
            p[1] = TestDeployer.TroveManagerParams(
                1.6 ether, 1.2 ether, 1.2 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
            );
        }
        if (n > 2) {
        ...
        if (n > 3) {
            p[3] = TestDeployer.TroveManagerParams(
                1.6 ether, 1.25 ether, 1.25 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
            );
        }

        TestDeployer deployer = new TestDeployer();
        TestDeployer.DeployedContractsMultiColl memory deploy = deployer.deployAndConnectContractsMultiColl(p);
        Contracts memory contracts;
        contracts.branches = deploy.contractsArray;
        contracts.collateralRegistry = deploy.collateralRegistry;
        contracts.boldToken = deploy.boldToken;
        contracts.hintHelpers = deploy.hintHelpers;
        contracts.weth = deploy.WETH;
