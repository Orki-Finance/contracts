...
import {QuillTestDeployer} from "./QuillDeployment.t.sol";

import {QuillAccessManagerUpgradeable} from "src/Quill/QuillAccessManagerUpgradeable.sol";

contract QuillTestSetup is BaseTest {
    ...
    function setUp() public virtual {
        ...
        addrDeployer = address(deployer);
        QuillTestDeployer.LiquityContractsDev memory contracts;
        QuillTestDeployer.Zappers memory zappers;
        QuillTestDeployer.DeployedContracts memory deploy = deployer.deployAndConnectContracts();
        ...
        WETH = deploy.WETH;
        zappers = deploy.zappers;
        sequencerSentinel = deploy.sequencerSentinel;
    ...
    function _redeemAndCreateZombieTrovesAAndB(ABCDEF memory _troveIDs) internal {
        ...
        uint256 redeemFromB = troveManager.getTroveEntireDebt(_troveIDs.B) - troveManager.MIN_DEBT() / 2;
        ...
        assertLt(troveManager.getTroveEntireDebt(_troveIDs.B), troveManager.MIN_DEBT());
    ...
    function _redeemAndCreateZombieTroveAAndHitB(ABCDEF memory _troveIDs) internal {
        ...
        // Check A has debt == 0, and B has debt > min_debt;
        assertEq(troveManager.getTroveEntireDebt(_troveIDs.A), 0);
        assertGt(troveManager.getTroveEntireDebt(_troveIDs.B), troveManager.MIN_DEBT());
    ...
    function _getSPYield(uint256 _aggInterest) internal view returns (uint256) {
        uint256 spYield = activePool.stabilityPoolYieldSplit() * _aggInterest / 1e18;
        assertGt(spYield, 0);
