...
contract Redemptions is DevTestSetup {
    ...
    function testZombieTrovePointerGetsResetIfTroveIsResuscitatedManuallyByOwner() public {
        ...
        // Restore trove
        adjustZombieTrove(B, troveIDs.B, 0, false, troveManager.MIN_DEBT(), true);

        // Check B is above min debt
        assertGt(troveManager.getTroveEntireDebt(troveIDs.B), troveManager.MIN_DEBT(), "B debt should be above min");
    ...
    function testZombieTrovesCanAccrueInterestThatBringThemAboveMIN_DEBT() public {
        ...
        assertLt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt);
        assertLt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt);
        ...
        assertGt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt, "B debt should be more than min");
    }
    ...
    function testZombieTrovesCanReceiveRedistGainsThatBringThemAboveMIN_DEBT() public {
        ...
        uint256 mindebt = troveManager.MIN_DEBT();
        assertLt(troveManager.getTroveEntireDebt(troveIDs.A), mindebt);
        assertLt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt);
        ...
        assertGt(troveManager.getTroveEntireDebt(troveIDs.A), mindebt);
        assertGt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt);
    }
    ...
    function testZombieBorrowerCanDrawFreshDebtToAboveMIN_DEBT() public {
        ...
        uint256 debtDelta_A = mindebt - troveManager.getTroveEntireDebt(troveIDs.A);
        uint256 debtDelta_B = mindebt - troveManager.getTroveEntireDebt(troveIDs.B);
    ...
    function testZombieBorrowerDrawsFreshDebtToAboveMIN_DEBTReducesPendingInterestTo0() public {
        ...
        // Check they are above the min debt
        assertGt(troveManager.getTroveEntireDebt(troveIDs.A), mindebt, "A debt should be above min");
        assertGt(troveManager.getTroveEntireDebt(troveIDs.B), mindebt, "B debt should be above min");
    ...
    function testZombieTroveBorrowerCanNotDrawFreshDebtToBelowMIN_DEBT() public {
        ...
        (uint256 borrow_A,) = findAmountToBorrowWithAdjustTrove(troveIDs.A, mindebt - debtDeficiency);
        (uint256 borrow_B,) = findAmountToBorrowWithAdjustTrove(troveIDs.B, mindebt - debtDeficiency);
    ...
    function testRedemptionAfterRedistributionInBatch() public {
        ...
        uint256 BTroveId = openTroveAndJoinBatchManager(B, 100 ether, 10e21, B, troveManager.minAnnualInterestRate());
        uint256 ATroveId = openTroveAndJoinBatchManager(A, 100 ether, 10e21, B, troveManager.minAnnualInterestRate());
        uint256 CTroveId = openTroveNoHints100pct(C, 100 ether, 100e21, troveManager.minAnnualInterestRate());
        ...
        assertApproxEqAbs(
            ...
            (ARecordedDebt + BRecordedDebt) * troveManager.minAnnualInterestRate() / DECIMAL_PRECISION,
            1,
    ...
    function testBaseRateDecayCannotBeSlowedDown() external {
        openTroveHelper({
            ...
            _annualInterestRate: troveManager.minAnnualInterestRate()
        });
