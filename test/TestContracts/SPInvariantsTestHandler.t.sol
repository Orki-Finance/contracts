...
contract SPInvariantsTestHandler is BaseHandler, TroveId {
    ...
    function openTrove(uint256 borrowed) external returns (uint256 debt) {
        ...
        debt = borrowed + hintHelpers.predictOpenTroveUpfrontFee(0, borrowed, minInterestRate);
        ...
        uint256 troveId = borrowerOperations.openTrove(
            msg.sender, i, coll, borrowed, 0, 0, minInterestRate, type(uint256).max, address(0), address(0), address(0)
        );
