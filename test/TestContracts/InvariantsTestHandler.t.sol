...
import {
    ...
    MIN_POSSIBLE_ANNUAL_INTEREST_RATE,
    MIN_BOLD_IN_SP,
    // MIN_DEBT,
...
uint256 constant INTEREST_RATE_MIN = MIN_POSSIBLE_ANNUAL_INTEREST_RATE - 1; // Sometimes try rates lower than the min
...
contract InvariantsTestHandler is Assertions, BaseHandler, BaseMultiCollateralTest {
    ...
    function _openTrove(OpenTroveContext memory v) internal {
        ...
        try _functionCaller.call(address(v.c.borrowerOperations), _encodeOpenTrove(v)) {
            ...
            if (v.join) {
                ...
                assertGeDecimal(
                    v.interestRate,
                    branches[v.i].troveManager.minAnnualInterestRate(),
                    18,
                    "Should have failed as rate < min"
                );
            ...
            assertGeDecimal(v.debt, v.c.troveManager.MIN_DEBT(), 18, "Should have failed as debt < min");
            ...
            if (selector == BorrowerOperations.InvalidInterestBatchManager.selector) {
                ...
                assertLtDecimal(
                    ...
                    branches[v.i].troveManager.minAnnualInterestRate(),
                    18,
                    "Shouldn't have failed as rate >= min"
                );
                ...
                assertLtDecimal(v.debt, v.c.troveManager.MIN_DEBT(), 18, "Shouldn't have failed as debt >= min");
    ...
    function adjustTrove(
        ...
        v.maxDebtDec = v.t.entireDebt > v.c.troveManager.MIN_DEBT() ? v.t.entireDebt - v.c.troveManager.MIN_DEBT() : 0;
        ...
        try _functionCaller.call(
            ...
            assertGeDecimal(v.newDebt, v.c.troveManager.MIN_DEBT(), 18, "Should have failed as new debt < MIN_DEBT");
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertLtDecimal(
                    v.newDebt, v.c.troveManager.MIN_DEBT(), 18, "Shouldn't have failed as new debt >= MIN_DEBT"
                );
        ...
        external
    {
        ...
        try v.c.borrowerOperations.adjustTroveInterestRate(
            ...
            assertGeDecimal(
                newInterestRate,
                branches[i].troveManager.minAnnualInterestRate(),
                18,
                "Should have failed as rate < min"
            );
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                info("New TCR would have been: ", newTCR.decimal());
            } else if (selector == IERC721Errors.ERC721NonexistentToken.selector) {
                assertFalse(_isOpen(i, v.troveId), "Open Trove should have an NFT");
            } else {
                revert(string.concat("Unexpected error: ", v.errorString));
            }
    ...
    function applyMyPendingDebt(uint256 i, uint32 upperHintSeed, uint32 lowerHintSeed) external {
        ...
        try v.c.borrowerOperations.applyPendingDebt(v.troveId, v.lowerHint, v.upperHint) {
            ...
            if (v.t.entireDebt >= v.c.troveManager.MIN_DEBT()) {
                _zombieTroveIds[i].remove(v.troveId);
                if (designatedVictimId[i] == v.troveId) designatedVictimId[i] = 0;
            }
    ...
    function registerBatchManager(
        ...
        try c.borrowerOperations.registerBatchManager(
            ...
            assertGeDecimal(
                ...
                branches[i].troveManager.minAnnualInterestRate(),
                18,
                "Wrong: min declared < min allowed"
            );
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertTrue(
                    minInterestRate < branches[i].troveManager.minAnnualInterestRate()
                        || maxInterestRate < branches[i].troveManager.minAnnualInterestRate(),
    ...
    function _mintYield(uint256 i, uint256 pendingInterest, uint256 upfrontFee) internal {
        ...
        uint256 mintedSPBoldYield = mintedYield * branches[i].activePool.stabilityPoolYieldSplit() / DECIMAL_PRECISION;
    ...
    {
        // uint256 mindebt = branches[i].troveManager.MIN_DEBT();
        ...
        r[i].redeemed.push(
            Redeemed({
                ...
                coll: collRedeemed,
                debt: debtRedeemed,
                becomesZombie: newDebt < branches[i].troveManager.MIN_DEBT()
            })
        );
        ...
        if (0 < newDebt && newDebt < branches[i].troveManager.MIN_DEBT()) r[i].newDesignatedVictimId = troveId;
    }
        ...
        returns (bytes4 selector, string memory errorString)
    {
        ...
        if (revertData.length == 4) {
            ...
            if (selector == BorrowerOperations.CallerNotPriceFeedOrCollateralRegistry.selector) {
                return (selector, "BorrowerOperations.CallerNotPriceFeedOrCollateralRegistry()");
            }
        ...
        if (revertData.length == 4 + 32) {
            ...
            if (selector == IERC721Errors.ERC721NonexistentToken.selector) {
                return (selector, string.concat("IERC721Errors.ERC721NonexistentToken(", uint256(param).decimal(), ")"));
            }
        }
