...
contract AnchoredInvariantsTest is Logging, BaseInvariantTest, BaseMultiCollateralTest, TroveId {
    ...
    function setUp() public override {
        ...
        p[0] = TestDeployer.TroveManagerParams(
            1.5 ether, 1.1 ether, 1.01 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
        );
        p[1] = TestDeployer.TroveManagerParams(
            1.6 ether, 1.2 ether, 1.01 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
        );
        p[2] = TestDeployer.TroveManagerParams(
            1.6 ether, 1.2 ether, 1.01 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
        );
        p[3] = TestDeployer.TroveManagerParams(
            1.6 ether, 1.25 ether, 1.01 ether, 0.1 ether, 0.05 ether, 0.1 ether, 2000e18, 72e16, _1pct / 2
        );
        TestDeployer deployer = new TestDeployer();
        Contracts memory contracts;
        TestDeployer.DeployedContractsMultiColl memory deploys = deployer.deployAndConnectContractsMultiColl(p);
        contracts.branches = deploys.contractsArray;
        contracts.collateralRegistry = deploys.collateralRegistry;
        contracts.boldToken = deploys.boldToken;
        contracts.hintHelpers = deploys.hintHelpers;
        contracts.weth = deploys.WETH;
