...
contract DeployOrkiSwellchain is DeployOrkiBase {
    ...
    address rand_oracle = 0x5C4c8d6f6Bf79B718F3e8399AaBdFEd01cB7e48f;
    ...
    address multisig = address(0x32fD58B8c4D454aB585DEA736b7D2b5929B6676c); // anvil#09
    address interestRouter = address(0x9c339BB827555AE214dF17b78c1aA28aCee183ce); // anvil#09
    ...
    function run() external {
        SALT = bytes32(uint256(30000000007467));
        ...
        initSlipstreamLiquidityPool(usdk_usdc_args);

        // USDK/ETH
        (uint256 wethPrice,) = r.contractsArray[0].priceFeed.fetchPrice();
        ...
        r.boldToken.approve(address(crocSwapDex), 2 * DECIMAL_PRECISION); // 2 usdq
        usdc.approve(address(crocSwapDex), 1 * 1e6); // 1 usdc
        initCrocSwapPool(crocSwapDex, r.boldToken, usdc, DECIMAL_PRECISION);
        initCrocSwapETHPool(crocSwapDex, r.boldToken, wethPrice);
